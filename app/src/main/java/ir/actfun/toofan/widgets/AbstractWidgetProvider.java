package ir.actfun.toofan.widgets;
...
public abstract class AbstractWidgetProvider extends AppWidgetProvider {
    ...
    protected Weather parseWidgetJson(String result, Context context) {
        try {
            ...
            widgetWeather.setWind(new DecimalFormat("#.0").format(wind) + " " + localize(sp, context, "speedUnit", "m/s")
                    + (widgetWeather.isWindDirectionAvailable() ? " " + MainActivity.getWindDirectionString(sp, context, widgetWeather) : ""));
            widgetWeather.setPressure(new DecimalFormat("#.0").format(pressure) + " " + localize(sp, context, "pressureUnit", "hPa"));
